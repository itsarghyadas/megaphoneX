...
export async function POST(req: NextRequest) {
  ...
  const authHeader = oauth.toHeader(
    oauth.authorize(
      {
        url: `https://api.twitter.com/2/tweets/${tweetId}/quote_tweets?expansions=author_id`,
        method: "GET",
      },
      tokenData
    )
  ...
  try {
    const response = await fetch(
      `https://api.twitter.com/2/tweets/${tweetId}/quote_tweets?expansions=author_id`,
      {
        headers: {
    ...
    const ids = data.includes.users.map((item: any) => item.id);
    console.log("quoted-ids", ids);
    const { result_count } = data.meta;
    return NextResponse.json({
      ids,
      result_count,
      message: "quoted ids are fetched successfully",
      success: true,
    });
  } catch (error: any) {
